...
def split(class_name,ratio, target_dir):
    ...
    list = os.listdir(base_dir + data_set + '/' + class_name)
    ...
    for key,image in enumerate(list):
        if key in test_files:
            shutil.copyfile(base_dir + data_set + '/' + class_name + '/' + image, test_dir + image)
        else:
            shutil.copyfile(base_dir + data_set + '/' + class_name + '/' + image, train_dir + image)

for class_name in tqdm(os.listdir(base_dir + data_set)):
    split(class_name, 0.2, base_dir_processed + data_set)